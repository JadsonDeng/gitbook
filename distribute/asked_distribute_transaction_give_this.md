...
事务提供一种机制，将一个活动涉及的所有操作纳入到一个不可分割的执行单元，组成事务的所有操作只有在所有操作均能正常执行的情况下方能提交，只要其中任一操作失败，都将导致整个事务的回滚。简单的说，事务提供一种“要么什么都不做，要么做全套（All or nothin）”机制。

## 数据库本地事务

### ACID

说到事务就不得不说，数据库事务的4大特性：
...
事务的一致性指的是，在一个事务执行前和执行后，数据都必须处于一致性状态。如果事务成功的完成，那么系统种所有变化将正确地应用，系统处于有效状态。如果在事务中出现错误，那么系统中的所有变化都将自动回滚，系统返回到原始状态。

+ I：隔离性(Isolation)

隔离性值得是在并发环境中，当不同的事务同时操作相同的数据时，每个事务都有各自的完整数据空间。由并发事务所做的修改必须与其他任何并发事务所做的修改隔离。事务查询数据更新时，数据所处的状态要么是另一事务修改它之前的状态，要么是另一事务修改它之后的状态，事务不会看到中间状态的数据。

打个比方，你买东西这个中间状态，是不影响其它人的。

+ D：持久性(Durability)

持久性指的是，只要事务成功结束，它对数据所做的更新就必须永久的保存下来。及时发生系统崩溃，重新启动数据库系统后，数据库还能恢复到事务成功结束时的状态。
...
InnoDB是mysql的一个存储引擎，大部分人对mysql都比较熟悉，这里简单介绍一下数据库事务实现的一些基本原理，在本地事务种，服务和资源在事务的包裹下可以看做是一体的。

![PNG](images/asked_distribute_transaction_give_this-1.png)
我们的本地事务由资源管理器进行管理：

![PNG](images/asked_distribute_transaction_give_this-2.png)
...
![PNG](images/asked_distribute_transaction_give_this-3.png)
这样的话就无法保证积分扣减了之后，优惠券扣减成功。

+ resource多个节点

同样的，互联网发展得太快了，我们的Mysql一般来说装千万级的数据就得进行分库分表，对于一个支付宝的转账业务来说，你给的朋友转钱，有可能你的数据库是在北京，而你的朋友的钱是存在上海，所以我们依然无法保证他们能同时成功。

![PNG](images/asked_distribute_transaction_give_this-4.png)
...
![PNG](images/asked_distribute_transaction_give_this-5.png)
...
![PNG](images/asked_distribute_transaction_give_this-6.png)
...
![PNG](images/asked_distribute_transaction_give_this-7.png)
...
![PNG](images/asked_distribute_transaction_give_this-8.png)
...
![PNG](images/asked_distribute_transaction_give_this-9.png)
