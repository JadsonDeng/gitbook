# Spring16: IOC 之开启 bean 的加载
---
![JPG](images/spring16-1.jpg)
...
第一个阶段前面已经用了 10 多篇博客深入分析了（[OC 之 IOC 初始化总结](spring15-ioc15-summary.md)）。所以从这篇开始分析第二个阶段：加载 bean 阶段。当我们显示或者隐式地调用 getBean() 时，则会触发加载 bean 阶段。如下：
```
